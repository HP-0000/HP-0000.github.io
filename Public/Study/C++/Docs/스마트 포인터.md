...
### 선언만 봐서는 객체를 가리키는지 배열을 가리키는지 구분X
...
### 선언만 봐서는 포인터가 객체를 소유하고 있는지 여부를 알 수 없다
...
책임 소재가 불분명해집니다.
### 포인터가 가리키는 객체를 어떻게 파괴해야 하는지 알 수 없다
...
### 코드의 모든 경로에서 파괴가 정확히 한 번 일어남을 보장할 수 없다
...
```
예외가 발생하거나, 여러 `return`문이 있는 복잡한 함수에서는 
메모리 해제를 빠뜨리기 쉽고 포인터를 실수로 두 번 해제하는 경우가 발생할 수 있습니다.
### 포인터가 가리키는 객체가 파괴되었는지 알 수 없다
```
...
이 문제들을 방지하기 위해 다음과 같이 포인터를 감싸 사용한다.
[unique_ptr, shared_ptr, weak_ptr](Public/Study/C++/Docs/unique_ptr,%20shared_ptr,%20weak_ptr.md)
